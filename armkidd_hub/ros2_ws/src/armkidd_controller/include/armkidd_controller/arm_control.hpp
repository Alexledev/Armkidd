...
class Arm_Control 
{
    public:
        ...
        {                
            switch(state)
            {
                ...
                case TransmitStates::GripperGraspComplete:
                    ...
                    RCLCPP_INFO(rclcpp::get_logger("ArmkiddInterface"), "Gripper grasp success...");
                    break;
        ...
        void setCurrentState(ArmMoveStates state)
        {
            ...
            if (current_state == gripper_grasping)
            {
                transmission_type = TransmitStates::GripperGrasp;
            }
        }
        ...
        void parseData(const std::vector<double>& joint_traj, std::vector<float>& traj_vec, bool isDone)
        {
            traj_vec = {
                static_cast<float>(isDone ? transmission_type : TransmitStates::ArmMoving),
                static_cast<float>(joint_traj[0]),
                static_cast<float>(joint_traj[1]),
                static_cast<float>(joint_traj[2] - 0.16), // ~ 15 degrees - 0.2618  - 0.3141
                static_cast<float>(joint_traj[3]),     
                static_cast<float>(joint_traj[4] + 0.1),    
                static_cast<float>(joint_traj[5] * -5.5),      
            };  
        }
